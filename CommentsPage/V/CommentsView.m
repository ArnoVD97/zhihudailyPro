...
- (void)initComment {
    ...
    _longLabelHeightArray = [[NSMutableArray alloc] init];
    _shortLabelHeightArray = [[NSMutableArray alloc] init];
    _longBoolArray = [[NSMutableArray alloc] init];
    _shortBoolArray = [[NSMutableArray alloc] init];
    _allBoolArray = [[NSMutableArray alloc] init];
    for (int i = 0; i < [_longDictionary[@"comments"] count]; i++) {
        label.text = _longDictionary[@"comments"][i][@"content"];
        label.numberOfLines = 0;
        label.textAlignment = NSTextAlignmentLeft;
        label.font = [UIFont systemFontOfSize:18];
        if (_longDictionary[@"comments"][i][@"reply_to"][@"content"] != nil) {
            [_longLabelHeightArray addObject:[NSString stringWithFormat:@"%lf", [label sizeThatFits:CGSizeMake(0.89 * SIZE_WIDTH - 50, CGFLOAT_MAX)].height + 35]];
            label.font = [UIFont systemFontOfSize:17];
            label.text = _longDictionary[@"comments"][i][@"reply_to"][@"content"];
            [_longBoolArray addObject:[NSString stringWithFormat:@"%lf", [label sizeThatFits:CGSizeMake(0.89 * SIZE_WIDTH - 50, CGFLOAT_MAX)].height]];
            ...
            [_longLabelHeightArray addObject:[NSString stringWithFormat:@"%lf", [label sizeThatFits:CGSizeMake(0.89 * SIZE_WIDTH - 50, CGFLOAT_MAX)].height]];
            [_longBoolArray addObject:@"0"];
            [_allBoolArray addObject:@"0"];
        }
...
- (void)initTableView {
    ...
    [self.tableView registerClass:[LongCommentTableViewCell class] forCellReuseIdentifier:@"long"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        if (indexPath.row == 0) {
            ...
            LongCommentTableViewCell *cell = [self.tableView dequeueReusableCellWithIdentifier:@"long" forIndexPath:indexPath];
...
- (void)pressExpand:(UIButton *)button {
    if (button.tintColor != [UIColor clearColor]) {
        ...
        if ([_allBoolArray[button.tag] isEqualToString: @"0"]) {
            if ([_longDictionary[@"comments"] count] == 0) {
                _shortLabelHeightArray[button.tag] = [NSString stringWithFormat:@"%lf", [_shortBoolArray[button.tag] doubleValue] + [_shortLabelHeightArray[button.tag] doubleValue]];
            } else {
                if (button.tag < [_longDictionary[@"comments"] count]) {
                    _longLabelHeightArray[button.tag] = [NSString stringWithFormat:@"%lf", [_longBoolArray[button.tag] doubleValue] + [_longLabelHeightArray[button.tag] doubleValue]];
                } else {
                    _shortLabelHeightArray[button.tag - [_longDictionary[@"comments"] count]] = [NSString stringWithFormat:@"%lf", [_shortBoolArray[button.tag - [_longDictionary[@"comments"] count]] doubleValue] + [_shortLabelHeightArray[button.tag - [_longDictionary[@"comments"] count]] doubleValue]];

                }
    
            }
            _allBoolArray[button.tag] = @"1";
        } else {
            if ([_longDictionary[@"comments"] count] == 0) {
                _shortLabelHeightArray[button.tag] = [NSString stringWithFormat:@"%lf", -[_shortBoolArray[button.tag] doubleValue] + [_shortLabelHeightArray[button.tag] doubleValue]];
            } else {
