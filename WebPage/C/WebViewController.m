...
@property (nonatomic, assign) CollectionView* collectionView;
...
- (void)viewDidLoad {
    ...
    _wkWebView = [[WebView alloc] initWithFrame:self.view.bounds];
...
//        [_collectionView.tableView reloadData];
        ...
        int goodNum;
        if (button.tag == 3) {
            [button setImage:[UIImage imageNamed:@"Like2.png"] forState:UIControlStateNormal];
            button.tag += 100;
...
- (void)getNetworkRequest:(id)extraId {
    [[Manager sharedManger] NetExtraWorkWithData:^(WebModel * _Nonnull mainViewModel) {
...
- (void)queryData {
    ...
    if ([self.collectionDatabase open]) {
        ...
        while ([collectionResultSet next]) {
            ...
            if (nowInt == idInt) {
                ...
                [self.wkWebView.collectButton setImage:[UIImage imageNamed:@"collection2.png"] forState:UIControlStateNormal];
                collect = 1;
            }
        }
    ...
    if ([self.goodDatabase open]) {
        ...
        while ([goodResultSet next]) {
            ...
            if (idInt == nowInt) {
                ...
                [self.wkWebView.goodButton setImage:[UIImage imageNamed:@"Like2.png"] forState:UIControlStateNormal];
    ...
    if (good == 0) {
        ...
        [self.wkWebView.goodButton setImage:[UIImage imageNamed:@"Like.png"] forState:UIControlStateNormal];
        int goodNum = [self.wkWebView.goodLabel.text intValue];
        self.wkWebView.goodLabel.text = [NSString stringWithFormat:@"%d", goodNum];
    }
    if (collect == 0) {
        self.wkWebView.collectButton.tag = 4;
        [self.wkWebView.collectButton setImage:[UIImage imageNamed:@"collection.png"] forState:UIControlStateNormal];
    }
}
